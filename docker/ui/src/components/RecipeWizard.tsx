...
import { fetchRecipeLines, fetchRecipeRules } from '../api'
import { fmt, fmtISK } from '../utils'
import type { Recipe, RecipeLine, RecipeRule, DraftLine } from '../types'
...
export default function RecipeWizard({ recipe, onAdd, onBack }: Props) {
    ...
    Promise.all([fetchRecipeLines(recipe.id), fetchRecipeRules(recipe.id)])
      .then(([ls, rs]) => {
        ...
        ls.forEach(l => {
          ...
          initEnabled[l.id] = !l.is_optional   // required lines always on
        })
        setQtys(initQtys)
  ...
  function handleQtyChange(lineId: number, qty: number) {
    setQtys(prev => {
      ...
      // Apply 'multiplies' rules originating from this line
  ...
  function handleToggle(lineId: number, on: boolean) {
    setEnabled(prev => {
      ...
      // Apply 'requires' rules: enabling trigger force-enables target
